...
Ext.define('Ext.grid.column.Column', {
     ...
     * A renderer is an 'interceptor' method which can be used transform data (value, appearance, etc.) before it is
     * rendered. Example:
     *
     *     {
     ...
     *
     * The renderer function returns the HTML string to be rendered.
     */
    renderer: false,
    ...
    applyColumnState: function (state) {
        ...
        if (Ext.isDefined(state.hidden)) {
            me.hidden = state.hidden;
        }
        if (Ext.isDefined(state.locked)) {
            me.locked = state.locked;
        }
        if (Ext.isDefined(state.sortable)) {
            me.sortable = state.sortable;
        }
        if (Ext.isDefined(state.width)) {
            delete me.flex;
            me.width = state.width;
        } else if (Ext.isDefined(state.flex)) {
            delete me.width;
            me.flex = state.flex;
        }
    },
    ...
    onRender: function() {
        ...
        if ((!me.sortable || grid.sortableColumns === false) && !me.groupable && !me.lockable && (grid.enableColumnHide === false || !me.getOwnerHeaderCt().getHideableColumns().length)) {
